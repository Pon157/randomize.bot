...
# --- 2. БАЗА ДАННЫХ (Облегченная версия) ---
def init_db():
...
# --- 5. ОБРАБОТЧИК START (С Deep Linking) ---
...
async def cmd_start(message: Message, command: CommandObject, state: FSMContext):
    ...
    if command.args and command.args.startswith("lot_"):
        ...
        # Проверка времени финиша "на лету"
        ...
        if db_query("SELECT 1 FROM participants WHERE user_id = ? AND lot_id = ?", (user_id, lot_id), fetchone=True):
            return await message.answer("✅ Ты уже участвуешь в лотерее #" + str(lot_id))
        ...
        for ch in channels:
            try:
                ...
                if m.status in ["left", "kicked"]:
                    ...
                    return await message.answer(f"❌ Подпишись на канал {ch} и нажми кнопку снова!", reply_markup=kb)
            except: continue

        db_query("INSERT OR IGNORE INTO participants (user_id, lot_id) VALUES (?, ?)", (user_id, lot_id), commit=True)
        await message.answer(f"🎉 Ты успешно зарегистрирован в лотерее #{lot_id}!")
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📢 Наш канал", url=f"https://t.me/{LOT_CHANNEL.replace('@','')}")]
    ])
    await message.answer(f"Привет, {message.from_user.first_name}! 👋\n\nИщи активные розыгрыши в нашем канале {LOT_CHANNEL}.", reply_markup=kb)

# --- 6. PR АНКЕТА (ИСПРАВЛЕННАЯ) ---
@dp.callback_query(F.data == "apply_pr")
async def pr_start(c: CallbackQuery, state: FSMContext):
    await state.clear() 
    await c.message.answer("📝 **Анкета PR-менеджера**\n\nШаг 1: Твой возраст?")
...
async def pr_age(m: Message, state: FSMContext):
    ...
    await m.answer("Шаг 2: Твой ник в Telegram (через @)?")
...
async def pr_nick(m: Message, state: FSMContext):
    ...
    await m.answer("Шаг 3: Количество чатов для пиара?")
...
async def pr_chats(m: Message, state: FSMContext):
    await state.update_data(chats=m.text)
    await m.answer("Шаг 4: Скинь скриншот-пруф (одним фото):")
...
async def pr_end(m: Message, state: FSMContext):
    ...
    await m.answer("✅ Твоя заявка отправлена админу!")
    await state.clear()

# --- 7. АДМИНКА (Менеджер лотерей) ---
...
async def admin_list(callback: CallbackQuery):
    ...
    if not lots: return await callback.answer("Нет активных лотерей", show_alert=True)
    kb = [[InlineKeyboardButton(text=f"Лот #{l['id']} ({l['finish_value']})", callback_data=f"edit_{l['id']}")] for l in lots]
    kb.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="admin_back")])
    await callback.message.edit_text("Выбери лотерею для редактирования или стопа:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def edit_menu(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📝 Изменить Текст", callback_data=f"upd_text_{lot_id}"), InlineKeyboardButton(text="🔗 Каналы", callback_data=f"upd_ch_{lot_id}")],
        [InlineKeyboardButton(text="⏰ Смена условий", callback_data=f"upd_val_{lot_id}"), InlineKeyboardButton(text="🛑 Стоп", callback_data=f"stop_{lot_id}")],
...
async def start_update(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Пришли новое значение (текст или число):")
...
async def process_update(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Параметры обновлены!"); await state.clear()
...
async def stop_now(callback: CallbackQuery):
    ...
    await callback.answer("Лотерея завершена!"); await admin_list(callback)

# --- 8. СОЗДАНИЕ ЛОТЕРЕИ ---
@dp.callback_query(F.data == "admin_create", F.from_user.id.in_(ADMIN_IDS))
async def cl_init(c: CallbackQuery, s: FSMContext):
    await c.message.answer("1. Отправь текст (с оформлением и эмодзи):"); await s.set_state(CreateLot.text); await c.answer()
...
async def cl_text(m: Message, s: FSMContext):
    ...
    await s.update_data(t=m.text, e=ents); await m.answer("2. Каналы через пробел (@ch1 @ch2):"); await s.set_state(CreateLot.channels)

@dp.message(CreateLot.channels)
async def cl_ch(m: Message, s: FSMContext):
    await s.update_data(c=m.text.replace(" ", ",")); await m.answer("3. Тип (time или count):"); await s.set_state(CreateLot.finish_type)

@dp.message(CreateLot.finish_type)
async def cl_ft(m: Message, s: FSMContext):
    await s.update_data(ft=m.text); await m.answer("4. Значение (Дата ДД.ММ.ГГГГ ЧЧ:ММ или Кол-во людей):"); await s.set_state(CreateLot.value)
...
async def cl_final(m: Message, s: FSMContext):
    ...
    await m.answer(f"🚀 Лотерея #{lot_id} запущена!"); await s.clear()
