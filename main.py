...
import asyncio
import sys
import html as pyhtml  # <--- ВАЖНО: Переименовал, чтобы не конфликтовало с aiogram
...
# Используем значение из переменной окружения, если оно есть,
# иначе используйте значение по умолчанию

TOKEN = os.getenv("BOT_TOKEN")
ADMIN_IDS = [int(x.strip()) for x in os.getenv("ADMIN_IDS", "7653383471").split(",") if x.strip()]
PR_CHAT_ID = int(os.getenv("PR_CHAT_ID", "0"))
LOT_CHANNEL = os.getenv("LOT_CHANNEL", "https://t.me/+i58o6vqDwUZlMjcy")
...
async def run_final_selection(lot_id: int):
    ...
    for winner in winners_list:
        ...
        mention = f"@{winner['username']}" if winner[
            'username'] else f"<a href='tg://user?id={winner['user_id']}'>{safe_name}</a>"
...
async def cmd_start(message: Message, command: CommandObject, state: FSMContext):
    ...
    username = message.from_user.first_name or "друг"  # Используем имя или "друг" как запасной вариант
    ...
    if not user:
        ...
        if ref_id != 0:
            ...
            if inviter:
                # ИСПОЛЬЗУЕМ pyhtml
                await message.answer(f"👋 Вы приглашены партнером: <b>{pyhtml.escape(inviter['full_name'])}</b>",
                                     parse_mode="HTML")
                try:
                    await bot.send_message(ref_id,
                                           f"🤝 <b>У вас новый реферал!</b>\nПользователь: {pyhtml.escape(message.from_user.full_name)}",
                                           parse_mode="HTML")
                except:
                    pass
    ...
    if args and args.startswith("lot_"):
        try:
            ...
            check_exist = db_query("SELECT 1 FROM participants WHERE user_id = ? AND lot_id = ?", (uid, lot_id),
                                   fetchone=True)
            if check_exist:
            ...
            new_count_res = db_query("SELECT COUNT(*) as c FROM participants WHERE lot_id = ?", (lot_id,),
                                     fetchone=True)
            ...
            return await message.answer(
                f"✅ <b>УСПЕХ!</b> Вы зарегистрированы в розыгрыше #{lot_id}. Удачи! \n<b>Буст в нашем канале {LOT_CHANNEL} дает +50% к удаче</b>",
                parse_mode="HTML")
    ...
    text_hello = f"Хай, {username}! Я бот, который может проводить розыгрыши, опросы, викторины на различные призы. Скорей опробуй меня первым!"
...
async def process_view_reviews(c: CallbackQuery):
    ...
    if not reviews:
        await c.message.edit_text("💬 Отзывов пока нет. Станьте первым победителем и напишите!",
                                  reply_markup=kb.as_markup())
        return
...
async def show_refs_list(c: CallbackQuery):
    uid = c.from_user.id
    refs = db_query(
        "SELECT full_name, username, created_at FROM users WHERE referrer_id = ? ORDER BY created_at DESC LIMIT 40",
        (uid,), fetchall=True)
...
async def pr_step1(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text("📝 <b>ЗАЯВКА НА Пиар-менеджера</b>\n\n1. Напишите ваш возраст:", parse_mode="HTML")
...
async def pr_step2(m: Message, state: FSMContext):
    ...
    await m.answer("2. Пришлите ваш юзернейм")
...
async def pr_step3(m: Message, state: FSMContext):
    ...
    await m.answer("3. Пришлите скриншот вашей рассылки")
...
async def admin_main_menu(c: CallbackQuery):
    ...
    await c.message.edit_text("🛠 <b>АДМИН ПАНЕЛЬ</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb),
                              parse_mode="HTML")
...
async def admin_list_lots(c: CallbackQuery):
    ...
    if not lots:
        ...
        for l in lots:
            ...
            builder.button(text=f"{status_emoji} #{l['id']} (Уч: {l['participants_count']})",
                           callback_data=f"manage_{l['id']}")
...
async def admin_show_participants(c: CallbackQuery):
    lid = int(c.data.split("_")[1])
    parts = db_query("SELECT full_name, user_id, username FROM participants WHERE lot_id = ? LIMIT 60", (lid,),
                     fetchall=True)
...
async def edit_finish_init(c: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(EditLotState.new_value)
    prompt = "Введите новое время (в часах от сейчас):" if lot[
                                                               'finish_type'] == 'time' else "Введите новое кол-во участников:"
...
async def create_finish(m: Message, state: FSMContext):
    ...
    try:
        if post['photo']:
            ...
            sent = await bot.send_message(LOT_CHANNEL, "🎁 Новый розыгрыш! Жми кнопку ниже.",
                                          reply_markup=kb.as_markup())
        else:
